...
public class ThreadJoin {
	...
	//	List<Long> inputNumbers = Arrays.asList(1000000L, 234L, 24544L, 2048L, 23L, 1025L);
		...
		for(Thread thread: factorialThreads) {
			thread.join();
			// each thread wait for 2 second and after that terminated.
		//	thread.join(2000);
		}
		...
		System.out.println(Thread.currentThread().getName() + " thread is waiting until the completion of another thread");
